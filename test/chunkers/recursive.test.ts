...
describe('CJK chunking (v0.32.7)', () => {
  test('MARKDOWN_CHUNKER_VERSION is 5', async () => {
    // v5 applies the final estimated-token cap after structured wrappers
    // are added, so existing pages must be swept through the new chunk shape.
    const mod = await import('../../src/core/chunkers/recursive.ts');
    expect(mod.MARKDOWN_CHUNKER_VERSION).toBe(5);
  });
